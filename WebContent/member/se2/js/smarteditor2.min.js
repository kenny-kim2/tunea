...
nhn.W3CDOMRange = jindo
		.$Class({
			...
			insertNode : function(b) {
				...
				console.log(b.src);
				if(b.src!=undefined){
					var src2=b.src.replace("file:///C:/Users/Hyunhong/Desktop/TeamPrj/문서/페이지 구성/TuneaPrj/WebContent","http://localhost:8080/TuneaPrj");
					b.src=src2;
					b.style.width="100px";
		...
		.$Class({
			...
			_assignHTMLElements : function(b, a) {
				...
				if (typeof (a) === "undefined"
						|| (typeof (a.bUseModeChanger) === "undefined" || a.bUseModeChanger === true)) {
					this.elWYSIWYGButton.style.display = "block";
					this.elHTMLSrcButton.style.display = "block";
					this.elTEXTButton.style.display = "block"
				} else {
					if (typeof (a.bUseVerticalResizer) !== "undefined"
							|| a.bUseVerticalResizer === false) {
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	_assignHTMLElements : function(b, a) {
		...
		if (typeof (a) === "undefined"
				|| typeof (a.bUseVerticalResizer) === "undefined"
				|| a.bUseVerticalResizer === true) {
			this.oResizeGrip.style.display = "block"
		} else {
			if (typeof (a.bUseModeChanger) !== "undefined"
					&& a.bUseModeChanger === false) {
				this.elModeToolbar.style.display = "none"
			} else {
...
nhn.husky.SE2M_Accessibility = jindo.$Class({
	...
	_assignHTMLElements : function(a) {
		var b = a;
		this.elHelpPopupLayer = jindo.$$.getSingle("DIV.se2_accessibility", b);
	...
	$LOCAL_BEFORE_FIRST : function(a) {
		jindo.$Fn(
				jindo.$Fn(this.oApp.exec, this.oApp).bind("CLOSE_HELP_POPUP",
						[ this.oCloseButton ]), this).attach(this.oCloseButton,
				"click");
		jindo.$Fn(
				jindo.$Fn(this.oApp.exec, this.oApp).bind("CLOSE_HELP_POPUP",
						[ this.oCloseButton2 ]), this).attach(
				this.oCloseButton2, "click");
	...
	$ON_FOCUS_NEXT_ELEMENT : function() {
		if (!!this.htAccessOption.sNextElementId) {
			var a = this.htAccessOption.sNextElementId;
	...
	$ON_FOCUS_BEFORE_ELEMENT : function() {
		if (!!this.htAccessOption.sBeforeElementId) {
			var a = this.htAccessOption.sBeforeElementId;
			if (document.getElementById(a)) {
				if (jindo.$Agent().navigator().ie
						&& jindo.$Agent().navigator().version == 7) {
					window.focus();
					document.getElementById(a).focus()
				} else {
					document.getElementById(a).focus()
	...
	$ON_OPEN_HELP_POPUP : function() {
		...
		this.nCalcX = this.htTopLeftCorner.x + this.oApp.getEditingAreaWidth()
				- this.nLayerWidth;
...
nhn.husky.SE2M_Hyperlink = jindo
		.$Class({
			...
			irToDb : function(q) {
				var k = q.cloneNode(true);
				var l = this.oApp.getEmptySelection();
				var r = l._getFirstRealChild(k);
				var b = l._getLastRealChild(k);
				var n = jindo.$A(l._getNodesBetween(r, b));
				var d = n.filter(function(a) {
					return (a && a.nodeType === 3)
				}).$value();
				var m = d;
				var j = d;
				var h = this.oApp.getWYSIWYGDocument().createElement("DIV");
				var e, p;
				var c = "@" + (new Date()).getTime() + "@";
				var g = new RegExp(c, "g");
				for (var f = 0, o = d.length; f < o; f++) {
					e = m[f].parentNode;
					p = false;
					while (e) {
						if (e.tagName === "A" || e.tagName === "PRE") {
							p = true;
							break
						}
						e = e.parentNode
				...
				h = l = r = b = n = d = m = j = e = null;
				k.innerHTML = k.innerHTML.replace(g, "");
				q.innerHTML = k.innerHTML;
				k = null
			},
		...
		.$Class({
			...
			_wrapBlock : function(w, f) {
				...
				if (b.startContainer === b.endContainer
					...
					if (nhn.husky.SE2M_Utils.isBlankNode(b.startContainer)
							|| nhn.husky.SE2M_Utils.isFirstChildOfNode("IMG",
									b.startContainer.tagName, b.startContainer)) {
...
nhn.husky.SE2M_TableEditor = jindo
		.$Class({
			...
			_reassignCellSizes : function(s) {
				var d = new Array(2);
				d[0] = jindo.$$(">TBODY>TR>TD", s, {
					oneTimeOffCache : true
				});
				d[1] = jindo.$$(">TBODY>TR>TH", s, {
					oneTimeOffCache : true
				});
				var k = new Array(d[0].length + d[1].length);
				var c = 0;
				var j = this.parseIntOr0(s.border);
				var a = this.parseIntOr0(s.cellPadding);
				for (var o = 0; o < 2; o++) {
					for (var r = 0; r < d[o].length; r++) {
						var g = d[o][r];
						var b = jindo.$Element(g);
						var v = this.parseIntOr0(b.css("paddingLeft"));
						var q = this.parseIntOr0(b.css("paddingRight"));
						var w = this.parseIntOr0(b.css("paddingTop"));
						var u = this.parseIntOr0(b.css("paddingBottom"));
						var m = this.parseBorder(b.css("borderLeftWidth"), b
								.css("borderLeftStyle"));
						var f = this.parseBorder(b.css("borderRightWidth"), b
								.css("borderRightStyle"));
						var e = this.parseBorder(b.css("borderTopWidth"), b
								.css("borderTopStyle"));
						var x = this.parseBorder(b.css("borderBottomWidth"), b
								.css("borderBottomStyle"));
						var p, t;
						var h = jindo.$Agent().navigator();
						if (h.ie && h.nativeVersion >= 9) {
							p = g.offsetWidth + "px";
							t = g.offsetHeight - (e + x) + "px"
						} else {
							p = g.offsetWidth - (v + q + m + f) + "px";
							t = g.offsetHeight - (w + u + e + x) + "px"
						}
						k[c++] = [ g, p, t ]
					}
				}
				for (var r = 0; r < c; r++) {
					var l = k[r];
					l[0].removeAttribute("width");
					l[0].removeAttribute("height");
					l[0].style.width = l[1];
					l[0].style.height = l[2]
				}
